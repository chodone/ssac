...
> 포함관계에 대한 특성
>
> - 블럭요소: 다른 블럭요소, 인라인 요소, 컨텐츠 모두 포함 가능
> - 인라인 요소 : 다른 인라인 요소, 컨텐츠는 포함 가능, 블럭 요소는 포함 불가능
...
> - 하나의 HTML Element에 id이름을 여러 개 불일 수 없음.
>
> class 특성
>
...
> - Noonnu Fonr : https://noonnu.cc/ (한글)
...
> 박스배치 제어를 위한 display : flex 속성및 배치관련 속성을 부모요소에 적용함
...
> justigy-content, align-items 의 방향이 flex-direction의 설정에 의해서 뒤바뀔 수 있음

### 반응형 웹

> 반응형 웹, 적응형 웹
> OSMU(One Sourc Multi Use) 여부로 반응형과 적응형을 구분
...
> @media 키워드를 사용해서 break point를 설정을 하고, 그에 따른 독립된 코드 블럭을 실행 할 수 있게 하는 것
> break point(변경점)
...
=> PC는 1024 에서 1920 까지 코딩, phone은 320에서 640까지 코딩
=> 이렇게 코딩하면 지정한 범위 밖으로 나갔을 때 CSS가 적용되지 않으므로 잘 사용하지 않는다.


EX) PC 해상도(기준) : 해상도 설정 하지 않음 / tablet 생상도 : ~900px /phone 해상도 : ~ 640px
...
> clone : 복제
>
> - remote repository를 local repository롤 복제
>
> staging : commit할 파일을 선택하는 것(<-> unstaging)
> commit : staging된 파일을 파일의 업데이트를 Fix(확정)하는 것
> push : commit된 snapshot(업데이트 완료된 상태)을 remote repository에 업로드(업테이트) 하는 것
>
